	...
	def display(self):
		...
		for temp in self.danyaolist:
			...
			for i in temp:
				...
				i.display()
				i.move()
		for temp in tempremove:
			self.danyaolist.remove(temp) 
	def moveright(self):
		if self.x <= 375:
			self.x += 9
	def moveleft(self):
		if self.x >= 5:
			self.x -= 9
	def shoot(self):
		zidan1 = ZiDan(self.x+7,self.y+28,self.screen)
		zidan2 = ZiDan(self.x+70,self.y+28,self.screen)
		self.danyaolist.append([zidan1,zidan2])
class EnemyPlane:
	def __init__(self,screen):
		self.x = 200
		self.y = 0 
...
def dazhong(heroplane,enemyplane):
	...
	if heroplane and enemyplane:
		...
		for herobullets in heroplane.danyaolist:
			for herobullet in herobullets:
...
if __name__ == "__main__":
	...
	while True:
		...
		if heroplane:
			heroplane.display()
			dazhong(heroplane,enemyplane)
			if heroplane.imagenum >= 15:
				heroplane = None
			elif heroplane.imagenum >= 11:
				heroplane.image = pygame.image.load("./feiji/hero_blowup_n4.png").convert()
			elif heroplane.imagenum >= 7:
